...
///
/// `accept` is declared explicitly even though the reference launcher's `gate`/`login` calls send
/// neither (their content type is unset): a reqwest client merges its own default `Accept: */*` into
/// any request that omits it, and it does that after the point the fidelity check reads the built
/// request back, so an undeclared default is invisible to it. Declaring the identical `*/*` here keeps
/// the wire byte unchanged while bringing the header inside the check, the same pattern `bootver.rs`'s
/// `build_request` uses; `accept-encoding`'s value is exempt from the comparison (see
/// [`crate::NEGOTIATED_HEADERS`]).
fn build_request(
    url: Url,
    context: &FrontierContext<'_>,
    now: &LauncherTime,
