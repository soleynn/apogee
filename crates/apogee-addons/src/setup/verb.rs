...
//!
//! Every op kind is idempotent on its own: a registry write overwrites rather than adds, a removal
//! treats "it was not there" as success, and a file placement overwrites. That is deliberate rather
//! than incidental, and it is what the verb list is curated around. The prefix's own record
//! short-circuits a verb that has already run, but a record can be lost and a run can be interrupted
//! halfway, so the ops have to converge when they are repeated instead of relying on never being
//! repeated.
//!
//! A verb's effect is checked rather than assumed. Its ops returning success is not evidence that
//! anything landed, and what it wrote can be undone from outside by a runner upgrade. So a verb
//! states what should exist afterwards ([`Verb::verify`]), that is what decides whether the apply
...
//! different because of it. A [`VerbOp::Files`] can only be checked against `verify`: the
//! destination is a directory, and its existing says nothing about what was supposed to be inside
//! it. A registry op needs nothing stated, because it already declares its own key, name and value,
//! so [`stale`] derives the question from the op and asks the prefix's registry files directly.
//!
//! The post-apply check ([`apply`]) is the paths only, and deliberately so. Wine flushes its
//! registry asynchronously, some time after the program that wrote it exits, so reading the file
//! straight after `reg add` returns would report a value that landed as missing. What evidences the
//! write at that moment is `reg add`'s own exit status, which `registry_set` already reads. By the
//! time [`stale`] runs, on the next launch, nothing has run in the prefix and the file is the
//! answer.
...
///
/// # Errors
/// [`AddonError::VerbFailed`] wrapping whatever a registry op failed with, whatever a files op
/// failed with (passed through rather than wrapped, since [`artifact::install`] already reports
/// against the verb's name), or [`AddonError::VerbIncomplete`] naming the first path `verb` promised
/// that is not there once every op has reported success.
...
/// does.
///
/// Two sources of evidence, and only one of them is stated in the manifest. The paths `verb` names
/// are checked as [`missing`] checks them, and every registry op is checked against the prefix's own
...
///
/// A [`VerbOp::Files`] contributes nothing. It names a destination directory rather than what
/// belongs in it, so a directory that outlived its contents would read as intact; `verify` is what
/// makes a placement checkable, which is what it is for.
