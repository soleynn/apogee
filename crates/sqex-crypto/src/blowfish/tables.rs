// The tables below are transcribed verbatim from the published Blowfish specification and are
// checked against known-answer test vectors, not typed by hand for readability. Splitting each
// word with underscores would make it harder, not easier, to diff against the source.
#![allow(clippy::unreadable_literal)]

//! Standard Blowfish initialization constants: the fractional hexadecimal digits of
//! pi. Every Blowfish implementation seeds the P-array and the four S-boxes with these
//! exact values before the key schedule runs.

/// The 18-entry P-array, pre-key-schedule.
...
/// The four 256-entry S-boxes, pre-key-schedule.
